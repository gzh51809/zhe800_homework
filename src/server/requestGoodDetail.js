...
router.post('/requestGoodDetail', bodyParser.json(), async (request, response) => {
    let database = null;
    try {
        database = await connect();
        let result = await find({collection: database.db('zhe800').collection('goodDetail'), query: request.body});
