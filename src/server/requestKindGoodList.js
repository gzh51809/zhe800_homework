...
router.post('/requestKindGoodList', bodyParser.json(), async (request, response) => {
    let database = null;
    try {
        ...
        for (let i = 0; i < kindIds.length; i++) {
            let temp = await find({collection: database.db('zhe800').collection('kindgoodlist'), query: {kindId: kindIds[i]}});
