...
router.post('/requestBrandList', bodyParser.json(), async (request, response) => {
    let database = null;
    try {
        ...
        for (let i = 0; i < kindIds.length; i++) {
            let temp = await find({collection: database.db('zhe800').collection('brandList'), query: {kindId: kindIds[i]}});
